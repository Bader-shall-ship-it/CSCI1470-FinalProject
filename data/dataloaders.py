...
def CIFAR10_dataloader(augment: bool, batch_size: int, root: str = './dataset') -> Tuple[DataLoader, DataLoader]:
    ...
    train_dataset = torchvision.datasets.CIFAR10(root=root, train=True, download=True, transform=augment_CIFAR10 if augment else transform_CIFAR10_test)
    test_dataset = torchvision.datasets.CIFAR10(root=root, train=False, download=True, transform=transform_CIFAR10_test)
...
def ImageNet_dataloader(augment: bool, batch_size: int, root: str = './dataset') -> Tuple[DataLoader, DataLoader]:
    ...
    train_dataset = torchvision.datasets.ImageNet(root=root, train=True, download=True, transform=augment_ImageNet if augment else transform_ImageNet_test)
    test_dataset = torchvision.datasets.ImageNet(root=root, train=False, download=True, transform=transform_ImageNet_test)
