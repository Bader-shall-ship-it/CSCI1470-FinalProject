...
class NTXent(nn.Module):
    ...
    def forward(self, e_i, e_j):
        ...
        sim_matrix = F.cosine_similarity(representations.unsqueeze(
            1), representations.unsqueeze(0), dim=2)

        sim_ij = torch.diag(sim_matrix, self.batch_size)
        sim_ji = torch.diag(sim_matrix, -self.batch_size)
        positives, negatives = get_pos_and_neg_mask(
            self.batch_size, sim_ij, sim_ji)
        ...
        loss = torch.sum(-torch.log(nominator /
                         torch.sum(denominator, dim=1))) / (2.0 * self.batch_size)
